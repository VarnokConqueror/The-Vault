...
class PushStore {
  ...
  static Future<void> init() async {
    ...
    _recentEnvelopeIds = (prefs.getStringList(_prefRecentEnvelopes) ?? const <String>[])
        .map((e) => e.trim())
        .where((e) => e.isNotEmpty)
  ...
  static Future<bool> rememberEnvelopeId(String envelopeId) async {
    ...
    if (_recentEnvelopeIds.contains(id)) {
      return false;
    }
    _recentEnvelopeIds.add(id);
    if (_recentEnvelopeIds.length > 200) {
      _recentEnvelopeIds = _recentEnvelopeIds.sublist(_recentEnvelopeIds.length - 200);
    }
