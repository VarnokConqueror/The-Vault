...
class ContactsStore {
  ...
  static Future<bool> updateContact({
    ...
    final existingIndex =
        contactsNotifier.value.indexWhere((c) => c.id == trimmedId);
    if (existingIndex == -1) return false;

    final existing = contactsNotifier.value[existingIndex];
    final nameTrimmed = displayName.trim();
    final nextName = nameTrimmed.isEmpty ? 'Unknown' : nameTrimmed;
    if (existing.displayName == nextName) return true;

    // Keep ids stable here because other local stores reference contact ids.
    final updated = Contact(
      id: existing.id,
