...
class ChatAppearanceStore {
  ...
  static Future<void> setBackgroundEffects(
    String chatId, {
    double? brightness,
    double? blur,
  }) async {
    await _setAppearance(
      chatId,
  ...
  static Future<void> _setAppearance(
    ...
    Object? backgroundBrightness = _unset,
    Object? backgroundBlur = _unset,
    ...
    final nextBackgroundBrightness = backgroundBrightness == _unset
        ? _normalizeBackgroundBrightness(current?.backgroundBrightness)
        : _normalizeBackgroundBrightness(backgroundBrightness as double?);
    final nextBackgroundBlur = backgroundBlur == _unset
        ? _normalizeBackgroundBlur(current?.backgroundBlur)
        : _normalizeBackgroundBlur(backgroundBlur as double?);
    ...
    if (nextBackground == null &&
        nextBackgroundBrightness == null &&
        nextBackgroundBlur == null &&
        nextTone == null) {
      next.remove(id);
    } else {
      next[id] = ChatAppearance(
        ...
        backgroundBrightness: nextBackgroundBrightness,
  ...
  static double? _normalizeBackgroundBrightness(double? value) {
    if (value == null) return null;
    return value.clamp(0.0, 1.0);
  }

  static double? _normalizeBackgroundBlur(double? value) {
    if (value == null) return null;
    return value.clamp(0.0, 12.0);
  }
  ...
  static Map<String, ChatAppearance>? _parsePayload(
    ...
    for (final entry in decoded.entries) {
      ...
      if (value.containsKey('backgroundBrightness') &&
          value['backgroundBrightness'] != null &&
          value['backgroundBrightness'] is! num) {
        return null;
      }
      if (value.containsKey('backgroundBlur') &&
          value['backgroundBlur'] != null &&
          value['backgroundBlur'] is! num) {
        return null;
      }
      ...
      final backgroundBrightness = _normalizeBackgroundBrightness(
        (value['backgroundBrightness'] as num?)?.toDouble(),
      );
      final backgroundBlur = _normalizeBackgroundBlur(
        (value['backgroundBlur'] as num?)?.toDouble(),
      );
